...
- **Physical topology** – the actual layout of cables, wireless cells, and
  devices (star, bus, ring, mesh, etc.).
- **Logical topology** – how frames **logically flow** between nodes (for modern
  switched Ethernet this is usually a logical star, even if the cabling is
  different).

For troubleshooting, it’s important to know both where the cables go and how
traffic is logically forwarded.

### 6.2.2 WAN Topologies

Common WAN layouts:

- **Point-to-point** – a dedicated link between two sites.
- **Hub-and-spoke** – one central site links to multiple branch sites.
- **Partial or full mesh** – many sites connected together for redundancy.

Each choice impacts cost, redundancy, and routing complexity.

### 6.2.3 Point-to-Point WAN Topology

The simplest WAN topology:

- Two endpoints share a single link.  
- No media contention (only those two devices use the circuit).  
- Often used with leased lines and simple encapsulations like **PPP** or **HDLC**.

### 6.2.4 LAN Topologies

Typical LAN physical layouts:

- **Star** – each device connects to a central switch (most common).
- **Extended star** – multiple switches daisy-chained or in a hierarchy.
- **Full/partial mesh** – multiple links between switches for redundancy.

Logical forwarding is usually **star** (each host’s traffic goes through a
switch).

### 6.2.5 Half and Full Duplex Communication

- **Half-duplex** – devices can send or receive, but **not both at the same
  time** (like walkie-talkies). Shared media hubs and some older links.  
- **Full-duplex** – devices send and receive **simultaneously** (like a phone
  call). Modern switch ports and NICs.

Full-duplex removes collisions and allows better use of the link’s bandwidth.

### 6.2.6 Access Control Methods

Ways to control access to a shared medium:

- **Deterministic** – a device gets a turn based on some fixed rule (e.g.
  token-passing). Predictable, good for real-time traffic.
- **Contention-based** – any device can try to send; if collisions occur,
  devices use rules to back off and retry (Ethernet, Wi-Fi).

Layer 2 protocols choose an access method based on the medium and requirements.

### 6.2.7 Contention-Based Access – CSMA/CD

**CSMA/CD (Carrier Sense Multiple Access with Collision Detection)** – classic
Ethernet on shared media:

- A node “listens” to see if the medium is idle.
- If idle, it sends; if another device sends at the same time, there’s a
  **collision**.
- Devices detect collisions, send a jam signal, then wait a random backoff time
  and retry.

Modern **full-duplex switched Ethernet** doesn’t use CSMA/CD because collisions
don’t occur.
...
- Devices try to **avoid** collisions since they can’t detect them reliably in
  the air.
- A node waits for the medium to be idle, then uses random backoff timers and
  optional acknowledgements (ACKs) to reduce collision probability.

### 6.2.9 Check Your Understanding – Topologies

Use this section to test:

- Can I identify different **WAN and LAN topologies** from diagrams?
- Do I know when a link is **half vs full duplex**?
- Can I explain **CSMA/CD vs CSMA/CA** in my own words?

Add NetAcad quiz Q/A here once you’ve taken it.
...
A **frame** is the Layer 2 PDU. It includes:

- A **header** with addressing and control fields.
- A **payload** that carries the Layer 3 packet.
- A **trailer** (often including an FCS) for error detection.

Encapsulation into frames lets devices share a medium and detect corruption.

### 6.3.2 Frame Fields

Exact fields depend on the protocol (Ethernet, PPP, etc.), but usually include:

- **Destination MAC address**
- **Source MAC address**
- **Type/Length** – which protocol is in the payload.
- **Data** – the Layer 3 packet and any padding.
- **FCS** – a CRC value used for error detection.

If the FCS check fails, the frame is dropped.

### 6.3.3 Layer 2 Addresses

- Layer 2 uses **MAC addresses** (48-bit, usually written as six hex pairs).
- Source MAC = interface that **sent** the frame.  
- Destination MAC = interface that should **receive** the frame on that segment.

Routers strip and rebuild Layer 2 addresses at each hop; Layer 3 addresses stay
the same end-to-end.

### 6.3.4 LAN and WAN Frames

- **LAN frames** – Ethernet, Wi-Fi, etc. Typically include MAC addresses and
  support many hosts on a shared or switched medium.
- **WAN frames** – PPP, HDLC, Frame Relay, etc. Often used on point-to-point or
  provider networks and may have different addressing and control fields.

Different technologies = different frames, but all serve the same purpose:
**deliver the packet across one hop**.

### 6.3.5 Check Your Understanding – Data Link Frame

By this point I should be able to:

- Sketch a basic **Ethernet frame** and label major fields.
- Explain how **MAC addresses** work on a LAN.
- Describe why frames include an **FCS** and what happens when it fails.
...
- Layer 2 is the **bridge** between IP packets and the physical medium.
- LLC identifies the **Network layer protocol**; MAC does **encapsulation,
  addressing, and media access**.
- Different **topologies and access methods** change how devices share the
  medium and handle collisions.
- Frames wrap packets with all the information needed for **one-hop delivery**
  and basic error detection.
- Multiple organizations (IEEE, ITU, ISO, ANSI) define the **standards** that
  make multi-vendor networking possible.

I can now read Packet Tracer or Wireshark output and understand what’s happening
at Layer 2, not just Layer 3.

### 6.4.2 Module Quiz – Data Link Layer

This section is for:

- Writing down tricky quiz questions and their correct answers.  
- Noting any concepts I missed so I can revisit the relevant subsection.

(You can add the exact Q/A from NetAcad here after taking the quiz.)
